...
func TestDiffFilesForWorktree_StackedBase(t *testing.T) {
	...
	got := names(diffFilesForWorktree(context.Background(), clone, "feature-parent", "", "", ""))
	...
	got = names(diffFilesForWorktree(context.Background(), clone, "", "", "", ""))
	if !got["app/child.ts"] || !got["app/parent.ts"] {
	...
	got = names(diffFilesForWorktree(context.Background(), sb, "feature-parent", "", "", ""))
	if !got["app/child.ts"] || got["app/parent.ts"] {
