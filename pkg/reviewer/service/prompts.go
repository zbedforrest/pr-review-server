...
When the author says the finding is fixed (reply_class "resolution"), verify the fix in this checkout: concede when the code now handles the case the finding raised, hold when it still applies, abstain when you cannot tell. When the author accepts the finding without fixing it (deferring it to a ticket or a follow-up), abstain: there is nothing to verify and nothing to rebut.
