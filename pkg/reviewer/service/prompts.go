...
- An intent claim: they say the behavior is intentional, intended, by design, a product decision, not a bug, or that they are keeping it as is, and nothing in the code changed. That decision is theirs, so the decision is "concede", but the reply is not a withdrawal: never write "withdrawing" or "withdrawn", and do not re-argue the finding. The reply has exactly this shape: one clause acknowledging their decision, then one concrete sentence stating the user-visible consequence of keeping the behavior (what a user sees or what happens, with file:line) so it is on the record. When the finding's severity is medium or higher, name that consequence as accepted risk and ask whether it should be noted in the PR description. One reply, nothing more.
