...
func TestCreateReviewRunPersistsResolvedConfigAndIdempotency(t *testing.T) {
	...
	assert.Equal(t, reviewRunRetryAfter, replayRecorder.Header().Get("Retry-After"))
	assert.Equal(t, 1, githubCalls)
	assert.Equal(t, 1, apiPoller.prepareCalls)
	assert.Equal(t, 1, apiPoller.processCalls)

	getQueued := addReviewAPIUser(httptest.NewRequest(http.MethodGet, reviewRunsPathPrefix+job.RunID, nil), *userID)
	getQueuedRecorder := httptest.NewRecorder()
	s.handleReviewRunByID(getQueuedRecorder, getQueued)
	assert.Equal(t, http.StatusOK, getQueuedRecorder.Code)
	assert.Equal(t, reviewRunRetryAfter, getQueuedRecorder.Header().Get("Retry-After"))
...
func TestReviewRunGetAndCursorListExposeSafeMetadata(t *testing.T) {
	...
	require.NoError(t, database.UpsertReviewStageAttempt(&db.ReviewStageAttempt{
		...
		TurnBudgetUnit: runconfig.TurnBudgetUnitCompletedNonReasoningItem, TurnBudgetVersion: runconfig.TurnBudgetVersion,
		ErrorSummary: "CLI reported error in stream: API Error: 400 usage limits reached (log: /tmp/agent-logs/acme__widgets__pr42.jsonl)",
	}))
	...
	assert.Empty(t, getRecorder.Header().Get("Retry-After"))
	...
	assert.Contains(t, getRecorder.Body.String(), `"error_summary":"CLI reported error in stream: API Error: 400 usage limits reached (log: \u003cpath\u003e)"`)
	assert.NotContains(t, getRecorder.Body.String(), "/tmp/agent-logs")
